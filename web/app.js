...
async function loadPost(path){current=await api('/api/post?path='+encodeURIComponent(path));$('#path').value=current.path;$('#front').value=current.frontMatter;$('#body').value=current.body;dirty=false;showEditor();render();renderList()}
function openPost(path){requestDiscard(()=>loadPost(path))}
...
function openNewPost(){ $('#new-post-path').value='posts/new-post.md';$('#new-post-error').value='';showNewPost();setTimeout(()=>{$('#new-post-path').focus();$('#new-post-path').select()},0) }
function newPost(){requestDiscard(openNewPost)}
...
function showDiscard(show=true){$('#discard-modal').classList.toggle('hidden',!show)}
function requestDiscard(action){if(!dirty){action();return}pendingDiscardAction=action;showDiscard()}
function cancelDiscard(){pendingDiscardAction=null;showDiscard(false)}
$('#close-discard').onclick=$('#cancel-discard').onclick=cancelDiscard;
$('#discard-form').onsubmit=e=>{e.preventDefault();const action=pendingDiscardAction;pendingDiscardAction=null;showDiscard(false);if(action)action()}
...
document.addEventListener('keydown',e=>{if(e.key!=='Escape')return;if(!$('#discard-modal').classList.contains('hidden')){cancelDiscard();return}if(!$('#new-post-modal').classList.contains('hidden')){showNewPost(false);return}if(!$('#delete-post-modal').classList.contains('hidden')){showDeletePost(false);return}if(siteConfigured&&!$('#setup').classList.contains('hidden'))showSetup(false)});
